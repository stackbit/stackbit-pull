...
const writeFiles = response => {
  for (let i = 0; i < response.length; i++) {
    ...
    if (fs.existsSync(fullPath) && ['yml', 'yaml', 'toml', 'json'].includes(
...
const pullJsonFile = fileFullPath => require(fileFullPath)
const createFromJsonFile = combine(writeFiles, pullJsonFile)
      ...
      res.on('end', () => {
        ...
        if (res.statusCode >= 400) {
          return reject(new Error(
              `Failed to build project, statusCode: ${res.statusCode}, response: ${JSON.stringify(
                  response)}`))
        }
...
if (require.main === module) {
  commander
      .option('--stackbit-pull-api-url <stackbitPullApiUrl>',
          '[required] stackbit pull API URL')
      .option('--stackbit-api-key <stackbitApiKey>',
